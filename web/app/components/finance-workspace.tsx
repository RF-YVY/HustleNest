...
export function FinanceWorkspace({
  ...
  const rows = useMemo<FinanceRow[]>(() => {
    ...
    if (mode === "expenses") return expenses.filter((item) => !term || [item.category, item.notes, item.vendor?.name, item.description].join(" ").toLowerCase().includes(term));
      ...
      <section className="entity-workspace finance-workspace">
        ...
        <aside className="entity-detail">
          {selected ? <>
            ...
            {expense ? <>
              <div className="detail-section customer-summary"><div className="section-heading"><h3>Transaction context</h3></div><p><Factory size={15} /> {expense.vendor?.name || "No vendor assigned"}</p><p><CreditCard size={15} /> {expense.payment_method || "Payment method not recorded"}</p><p><Repeat2 size={15} /> {expense.is_recurring ? "Created from a recurring expense" : "One-time expense"}</p>{expense.description ? <p><ReceiptText size={15} /> {expense.description}</p> : null}</div>
