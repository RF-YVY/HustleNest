...
export function AppearanceSettingsCard({ settings, onUpdated }: Props) {
  ...
  const chooseTextScale = async (text_scale: number) => {
    setDraft((current) => ({ ...current, text_scale })); setWorking("text-size"); setMessage(null);
    try {
      const response = await fetch(`${bridgeUrl}/api/settings`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ section: "appearance", values: { text_scale }, expected_revision: settings.summary.revision }) });
      const payload = await response.json() as { ok: boolean; data?: SettingsWorkspaceData; error?: { message: string } };
      if (!response.ok || !payload.ok || !payload.data) throw new Error(payload.error?.message || "The text size could not be applied.");
      accept(payload.data, "Text size applied.");
    } catch (error) {
      setDraft((current) => ({ ...current, text_scale: settings.appearance.text_scale }));
      setMessage({ tone: "error", text: error instanceof Error ? error.message : "The text size could not be applied." });
    } finally { setWorking(""); }
  };
  ...
  return <article className="settings-card appearance-settings-card">
    ...
    <div className="appearance-grid">
      ...
      <section className="text-size-settings"><h3>Interface text size</h3><p className="theme-hint">Increase text throughout navigation, cards, forms, and tables. Changes apply immediately.</p><div className="text-size-preview" aria-hidden="true"><strong>Aa</strong><span>Preview text</span></div><div className="text-size-options">{textSizes.map((size) => <label key={size.value}><input type="radio" name="saved-text-size" checked={draft.text_scale === size.value} disabled={Boolean(working)} onChange={() => void chooseTextScale(size.value)} /><span><strong>{size.label}</strong><small>{size.detail}</small></span></label>)}</div></section>
