...
export function AppearanceSettingsCard({ settings, onUpdated }: Props) {
  ...
  return <article className="settings-card appearance-settings-card">
    ...
    <div className="appearance-grid">
      <section><h3>Color theme</h3><div className="theme-options"><label><input type="radio" name="saved-theme" checked={draft.theme === "light"} onChange={() => setDraft({ ...draft, theme: "light" })} /><span><i className="theme-swatch light" /><strong>Light</strong><small>Bright studio workspace</small></span></label><label><input type="radio" name="saved-theme" checked={draft.theme === "dark"} onChange={() => setDraft({ ...draft, theme: "dark" })} /><span><i className="theme-swatch dark" /><strong>Dark</strong><small>Low-light workspace</small></span></label></div></section>
