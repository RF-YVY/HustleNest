        ...
        <aside className="entity-detail">
          {selected ? (
            <>
              ...
              <div className="detail-section product-cost-breakdown"><div className="section-heading"><h3>Unit cost</h3><strong>{money(selected.unit_cost)}</strong></div><p><span>Base cost</span><strong>{money(selected.base_unit_cost)}</strong></p>{selected.cost_components.map((component, index) => <p key={`${component.label}-${index}`}><span>{component.label}</span><strong>{money(component.amount)}</strong></p>)}{!selected.cost_components.length ? <p className="quiet-empty">No extra unit costs configured.</p> : null}</div>
