...
import { Boxes, CircleAlert, Factory, FileText, Package, ReceiptText, Repeat2, Search, ShoppingBag, Sparkles, UserRound, X } from "lucide-react";
import { KeyboardEvent, useEffect, useMemo, useRef, useState } from "react";

export type GlobalSearchResult = {
  key: string;
  kind: "order" | "customer" | "product" | "material" | "vendor" | "expense" | "recurring" | "loss" | "document" | "action";
  title: string;
  subtitle: string;
  searchText: string;
  id: number | string;
...
const icons = { order: ShoppingBag, customer: UserRound, product: Package, material: Boxes, vendor: Factory, expense: ReceiptText, recurring: Repeat2, loss: CircleAlert, document: FileText, action: Sparkles };
const labels = { order: "Order", customer: "Customer", product: "Product", material: "Material", vendor: "Vendor", expense: "Expense", recurring: "Recurring", loss: "Loss", document: "Document", action: "Command" };
...
export function GlobalSearch({ results, onSelect }: { results: GlobalSearchResult[]; onSelect: (result: GlobalSearchResult) => void }) {
  ...
  const matches = useMemo(() => {
    ...
    if (!words.length) return results.filter((item) => item.kind === "action").slice(0, 6);
  ...
  return (
    <div className="global-search-wrap" data-testid="global-search" onBlur={(event) => { if (!(event.relatedTarget instanceof Node) || !event.currentTarget.contains(event.relatedTarget)) setOpen(false); }}>
      ...
      {open ? <div className="global-search-results" id="global-search-results" role="listbox">
        {!query.trim() ? <><div className="search-guidance"><Search size={19} /><div><strong>Find or do anything</strong><span>Search records, or start with a common command.</span></div></div>{matches.map((result, index) => { const Icon = icons[result.kind]; return <button type="button" role="option" aria-selected={index === activeIndex} className={index === activeIndex ? "search-result active" : "search-result"} key={result.key} onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(result)}><span className={`search-result-icon ${result.kind}`}><Icon size={17} /></span><span><strong>{result.title}</strong><small>{result.subtitle}</small></span><em>{labels[result.kind]}</em></button>; })}</> : matches.length ? matches.map((result, index) => {
