...
import { ChevronRight, CircleDot, Compass, House, LayoutGrid, Map as MapIcon, MapPin, MapPinned, PackageCheck, Search, ShoppingBag } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { bridgeUrl, type GeographyWorkspaceData } from "../lib/hustlenest";
import { DestinationDistributionMap } from "./destination-distribution-map";
...
export function GeographyWorkspace({ onOpenOrder, onOpenSettings }: { onOpenOrder: (id: number) => void; onOpenSettings: () => void }) {
  ...
  const chooseMapView = (view: "map" | "grid") => setMapView(view);
  ...
  return <div className="workspace geography-page">
    ...
    {error ? <div className="geography-empty"><MapPinned size={26} /><strong>Sales geography unavailable</strong><span>{error}</span></div> : <section className="geography-layout">
      <article className="state-map-card"><div className="geography-card-heading"><div><span>U.S. distribution</span><h2>{mapView === "map" ? "Destination map" : "Order activity by state"}</h2></div><div className="map-view-toggle" aria-label="Geography display"><button className={mapView === "map" ? "active" : ""} type="button" aria-pressed={mapView === "map"} onClick={() => chooseMapView("map")}><MapIcon size={14} />Map</button><button className={mapView === "grid" ? "active" : ""} type="button" aria-pressed={mapView === "grid"} onClick={() => chooseMapView("grid")}><LayoutGrid size={14} />State grid</button></div></div>{mapView === "map" ? <div className="destination-map"><DestinationDistributionMap destinations={data?.destinations ?? []} selectedKey={selected?.key ?? null} onSelect={setSelectedKey} /><div className="destination-map-caption"><MapPin size={14} /><span><small>Distribution view</small><strong>All shipped destinations</strong></span><em>{(data?.destinations ?? []).filter((destination) => destination.latitude !== null && destination.longitude !== null).length} locations</em></div></div> : <><div className="state-map-scroll"><div className="state-tile-map" aria-label="United States order distribution">{tiles.map(([code, column, row]) => { const count = stateCounts.get(code) ?? 0; const active = selectedState === code; const home = data?.home.state === code; const level = count ? Math.max(1, Math.ceil(count / maxCount * 4)) : 0; return <button className={`${count ? `has-orders level-${level}` : ""} ${active ? "active" : ""} ${home ? "home-state" : ""}`} style={{ gridColumn: column, gridRow: row }} onClick={() => chooseState(code)} title={`${code}: ${count} order${count === 1 ? "" : "s"}`} key={code}><strong>{code}</strong>{count ? <span>{count}</span> : null}{home ? <i><House size={8} /></i> : null}</button>; })}</div></div><div className="map-legend"><span><i /> No orders</span><span><i className="low" /> Emerging</span><span><i className="high" /> Strong activity</span>{data?.home.configured ? <span><House size={11} /> Home base</span> : null}</div></>}</article>
