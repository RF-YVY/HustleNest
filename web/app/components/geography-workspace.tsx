...
export function GeographyWorkspace({ onOpenOrder, onOpenSettings }: { onOpenOrder: (id: number) => void; onOpenSettings: () => void }) {
  ...
  const mapLocation = selected ? `${selected.city}, ${selected.state}` : selectedState === "All states" ? "United States" : data?.states.find((state) => state.code === selectedState)?.name ?? selectedState;
