...
export function MaterialsWorkspace({ materials, focusMaterialId, onEdit, onAdjust, onOpenProduct }: { materials: MaterialOption[]; focusMaterialId?: number | null; onEdit: (material: MaterialOption) => void; onAdjust: (material: MaterialOption) => void; onOpenProduct: (id: number) => void }) {
        ...
        <aside className="entity-detail">
          {current ? (
            <>
              ...
              <div className="detail-section"><div className="section-heading"><h3>Used by products</h3><span>{detail?.id === current.id ? detail.products.length : 0}</span></div>{detail?.id === current.id && detail.products.length ? detail.products.map((product) => <button className="related-order related-material" onClick={() => onOpenProduct(product.product_id)} key={product.product_id}><Boxes size={15} /><span><strong>{product.name}</strong><small>{product.sku} · {quantity(product.quantity_required)} {current.unit_of_measure || "units"} each · {product.include_in_unit_cost ? "Direct material" : "Track only"}</small></span><ChevronRight size={15} /></button>) : <p className="quiet-empty">No products currently use this material.</p>}</div>
