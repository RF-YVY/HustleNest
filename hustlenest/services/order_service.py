...
from . import crm_service, finance_service, goal_service, inventory_service

from PySide6.QtCore import QSizeF
from PySide6.QtGui import QFont, QGuiApplication, QPageSize, QPdfWriter, QTextDocument


_INVOICE_GUI_APPLICATION: Optional[QGuiApplication] = None
...
def _ensure_product_media_root() -> Path:
    root = get_storage_root() / "media" / "products"
...
def _is_managed_product_photo(path: Path) -> bool:
    try:
        path.relative_to(get_storage_root() / "media" / "products")
        return True
    except ValueError:
        return False
...
def _resolve_product_photo_path(value: str) -> Optional[Path]:
    ...
    if not raw.is_absolute():
        candidate = (get_storage_root() / value).expanduser()
        if candidate.exists():
            return candidate
    if raw.exists():
        return raw
...
def _absolute_product_photo_path(value: str) -> Optional[Path]:
    ...
    if raw.is_absolute():
        return raw
    return (get_storage_root() / value).expanduser()
...
def get_dashboard_snapshot() -> DashboardSnapshot:
    ...
    loss_total = finance_service.calculate_loss_total(start_date=fiscal_start, end_date=today)
    expense_total = finance_service.calculate_expense_total(start_date=fiscal_start, end_date=today)
    cash_flow = finance_service.summarize_cash_flow()
    materials_value = inventory_service.calculate_materials_value()
    inventory_alerts = inventory_service.build_inventory_alerts()
    crm_followups = crm_service.list_pending_followups()
    goal_status = goal_service.evaluate_goals()

    return replace(
        snapshot,
        product_breakdown=normalized_breakdown,
        loss_total=loss_total,
        expense_total=expense_total,
        materials_value=materials_value,
        cash_flow_projection_30=cash_flow.net_projection,
...
def list_distinct_customers() -> List[Tuple[str, str]]:
    return order_repository.fetch_distinct_customers()


def summarize_sales_tax(
    period_start: date,
    period_end: date,
    *,
    tax_rate_percent: Optional[float] = None,
    period_label: Optional[str] = None,
) -> Tuple[SalesTaxSummary, List[OrderReportRow]]:
    if period_end < period_start:
        raise ValueError("Tax summary end date cannot be before the start date.")

    orders = list(order_repository.fetch_order_report(period_start, period_end))

    settings = settings_repository.get_app_settings()
    rate_percent = tax_rate_percent
    if rate_percent is None:
        rate_percent = settings.tax_rate_percent
    rate_percent = max(0.0, float(rate_percent))
    rate_fraction = rate_percent / 100.0

    taxable_sales = sum(getattr(row, "total_amount", 0.0) for row in orders)
    display_sales = sum(row.display_total for row in orders)
    tax_collected = sum(getattr(row, "tax_amount", 0.0) for row in orders)

    expected_tax = round(taxable_sales * rate_fraction, 2)
    difference = round(tax_collected - expected_tax, 2)

    label = period_label or f"{period_start:%Y-%m-%d} to {period_end:%Y-%m-%d}"
    ...
    summary = SalesTaxSummary(
        ...
        display_sales=round(display_sales, 2),
        tax_rate_percent=round(rate_percent, 2),
...
def update_app_settings(settings: AppSettings) -> AppSettings:
    ...
    settings_repository.set_setting(
        "cloud_sync_enabled",
        "1" if settings.cloud_sync_enabled else "0",
    )
    settings_repository.set_setting(
        "cloud_sync_provider",
        settings.cloud_sync_provider.strip(),
    )
    interval_minutes = max(1, min(1440, int(settings.cloud_sync_interval_minutes)))
    settings_repository.set_setting("cloud_sync_interval_minutes", str(interval_minutes))
    config_payload = json.dumps(settings.cloud_sync_config or {}, ensure_ascii=False)
    settings_repository.set_setting("cloud_sync_settings_json", config_payload)
    return settings_repository.get_app_settings()


def update_cloud_sync_settings(
    *,
    enabled: bool,
    provider: str,
    ...
    settings_repository.set_setting("cloud_sync_enabled", "1" if enabled else "0")
    settings_repository.set_setting("cloud_sync_provider", provider.strip())
    clamped_interval = max(1, min(1440, int(interval_minutes)))
    settings_repository.set_setting("cloud_sync_interval_minutes", str(clamped_interval))
    settings_repository.set_setting("cloud_sync_settings_json", json.dumps(config, ensure_ascii=False))
    return settings_repository.get_app_settings()
...
def list_notifications(today: Optional[date] = None) -> List[NotificationMessage]:
    ...
    notifications: List[NotificationMessage] = list(inventory_service.build_inventory_alerts())
    notifications.extend(crm_service.build_followup_notifications())
...
def export_sales_tax_summary_csv(
    period_start: date,
    period_end: date,
    destination: str,
    *,
    ...
    summary, orders = summarize_sales_tax(
        period_start,
        period_end,
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["Business", settings.business_name])
        writer.writerow(["Period", summary.period_label])
        writer.writerow(["Start Date", summary.period_start.strftime("%Y-%m-%d")])
        writer.writerow(["End Date", summary.period_end.strftime("%Y-%m-%d")])
        writer.writerow(["Order Count", summary.order_count])
        writer.writerow(["Taxable Sales", f"{summary.taxable_sales:.2f}"])
        writer.writerow(["Displayed Sales", f"{summary.display_sales:.2f}"])
        writer.writerow(["Tax Rate (%)", f"{summary.tax_rate_percent:.2f}"])
        writer.writerow(["Tax Collected", f"{summary.tax_collected:.2f}"])
        writer.writerow(["Expected Tax", f"{summary.expected_tax:.2f}"])
        writer.writerow(["Difference", f"{summary.difference:.2f}"])
        writer.writerow([])
        writer.writerow([
            "Order #",
            "Date",
        ...
        for row in orders:
            ...
            writer.writerow(
                [
                    ...
                    row.status,
                    f"{row.total_amount:.2f}",
                    f"{getattr(row, 'tax_amount', 0.0):.2f}",
                    f"{row.display_total:.2f}",
                ]
            )
...
def export_sales_tax_summary_pdf(
    period_start: date,
    period_end: date,
    destination: str,
    *,
    ...
    settings = settings_repository.get_app_settings()
    html_content = _render_tax_summary_html(summary, orders, settings)
    _write_pdf_from_html(html_content, path)
    return path


def _render_tax_summary_html(
    summary: SalesTaxSummary,
    orders: List[OrderReportRow],
    settings: AppSettings,
) -> str:
    business_name = (settings.business_name or "").strip() or "Company Name"
    period_label = summary.period_label or "Sales Tax Summary"
    start_date = summary.period_start.strftime("%Y-%m-%d")
    end_date = summary.period_end.strftime("%Y-%m-%d")

    summary_rows = "".join(
        [
            f"<tr><th scope=\"row\">Taxable Sales</th><td>{_format_currency(summary.taxable_sales)}</td></tr>",
            f"<tr><th scope=\"row\">Displayed Sales</th><td>{_format_currency(summary.display_sales)}</td></tr>",
            f"<tr><th scope=\"row\">Tax Rate</th><td>{summary.tax_rate_percent:.2f}%</td></tr>",
            f"<tr><th scope=\"row\">Tax Collected</th><td>{_format_currency(summary.tax_collected)}</td></tr>",
            f"<tr><th scope=\"row\">Expected Tax</th><td>{_format_currency(summary.expected_tax)}</td></tr>",
            f"<tr><th scope=\"row\">Difference</th><td>{_format_signed_currency(summary.difference)}</td></tr>",
        ]
    )
    ...
    for order in orders:
        order_rows.append(
            "<tr>"
            f"<td>{html.escape(order.order_number)}</td>"
            f"<td>{order.order_date.strftime('%Y-%m-%d') if order.order_date else ''}</td>"
            f"<td>{html.escape(order.customer_name)}</td>"
            f"<td>{html.escape(order.status)}</td>"
            f"<td class=\"currency\">{_format_currency(order.total_amount)}</td>"
            f"<td class=\"currency\">{_format_currency(getattr(order, 'tax_amount', 0.0))}</td>"
            f"<td class=\"currency\">{_format_currency(order.display_total)}</td>"
            "</tr>"
        )

    if not order_rows:
        order_rows.append(
            "<tr><td colspan=\"7\" class=\"empty\">No invoices matched this period.</td></tr>"
        )

    styles = """
        body { font-family: 'Segoe UI', sans-serif; color: #263238; margin: 32px; }
        h1 { font-size: 24px; margin-bottom: 4px; }
        h2 { font-size: 18px; margin-top: 32px; margin-bottom: 12px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 16px; }
        th, td { padding: 8px 10px; border: 1px solid #cfd8dc; text-align: left; }
        th { background: #eceff1; font-weight: 600; }
        td.currency { text-align: right; font-variant-numeric: tabular-nums; }
        td.empty { text-align: center; font-style: italic; color: #546e7a; }
    ...
    <head>
        <meta charset=\"utf-8\" />
        <title>Sales Tax Summary</title>
        <style>{styles}</style>
    </head>
    <body>
        <h1>{html.escape(business_name)}</h1>
        <div class=\"meta\">
            <div><strong>Period:</strong> {html.escape(period_label)}</div>
            <div><strong>Dates:</strong> {start_date} – {end_date}</div>
            <div><strong>Invoices:</strong> {summary.order_count}</div>
        </div>
        <h2>Summary</h2>
        <table>
            <tbody>
        ...
        <p>Generated on {date.today().strftime('%Y-%m-%d')}.</p>
    </body>
    </html>
    """
...
def _write_pdf_from_html(html_content: str, path: Path) -> None:
    ensure_invoice_runtime()
    pdf_writer = QPdfWriter(str(path))
    pdf_writer.setPageSize(QPageSize(QPageSize.Letter))  # type: ignore[attr-defined]
    pdf_writer.setResolution(96)  # Use screen resolution (96 DPI) for proper text sizing

    document = QTextDocument()
    document.setDocumentMargin(36)
    document.setDefaultFont(QFont("Segoe UI", 12))  # Slightly larger base font
...
def ensure_invoice_runtime() -> None:
    """Initialize Qt's document/font runtime when invoices run without the desktop shell."""
    global _INVOICE_GUI_APPLICATION
    if QGuiApplication.instance() is None:
        _INVOICE_GUI_APPLICATION = QGuiApplication(["HustleNest Invoice Renderer"])
