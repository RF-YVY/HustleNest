...
def list_orders_in_range(start_date: date, end_date: date) -> List[Order]:
    """Fetch all orders within the specified date range (inclusive)."""
    with create_connection() as connection:
        cursor = connection.cursor()
        cursor.execute(
            """
...
def fetch_distinct_customers() -> List[Tuple[str, str]]:
    with create_connection() as connection:
        rows = connection.execute(
            """
            SELECT DISTINCT
                TRIM(customer_name) AS name,
                TRIM(IFNULL(customer_address, '')) AS address
            FROM orders
            WHERE TRIM(IFNULL(customer_name, '')) <> ''
            ORDER BY UPPER(TRIM(customer_name))
            """
        ).fetchall()

    customers: List[Tuple[str, str]] = []
    for row in rows:
        customers.append((row["name"] or "", row["address"] or ""))
    return customers
...
def fetch_order_by_number(order_number: str) -> Optional[Order]:
    """Fetch one active order by its unique, case-insensitive order number."""
    normalized = order_number.strip()
    if not normalized:
        return None
    with create_connection() as connection:
        row = connection.execute(
            "SELECT id FROM orders WHERE UPPER(order_number) = UPPER(?) AND deleted_at IS NULL LIMIT 1",
            (normalized,),
        ).fetchone()
    return fetch_order(int(row["id"])) if row is not None else None
...
def fetch_financial_totals() -> tuple[float, float, float, float, float]:
    with create_connection() as connection:
        ...
        for row in cursor.fetchall():
            ...
            if bool(row["is_paid"]):
                paid_sales += line_sales
            else:
                unpaid_sales += line_sales
        cursor.close()
    return total_sales, total_cost, freebie_cost, paid_sales, unpaid_sales
...
def get_product_sales_summary(start_date: date, end_date: date) -> List[ProductSalesSummary]:
    """Fetch product sales summary filtered by date range."""
    with create_connection() as connection:
        cursor = connection.execute(
            """
            SELECT
...
def get_customer_sales_summary(start_date: date, end_date: date) -> List[CustomerSalesSummary]:
    """Fetch customer sales summary filtered by date range."""
    with create_connection() as connection:
        cursor = connection.execute(
            """
            SELECT
...
def count_orders_in_range(start_date: date, end_date: date) -> int:
    """Count the number of orders within a date range."""
    with create_connection() as connection:
        cursor = connection.execute(
            """
            SELECT COUNT(*) AS cnt
...
def build_dashboard_snapshot() -> DashboardSnapshot:
    total_sales, total_cost, freebie_cost, paid_sales, unpaid_sales = fetch_financial_totals()
