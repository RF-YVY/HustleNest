...
def set_settings(values: Dict[str, str]) -> None:
    """Persist a validated group of settings as one transaction."""
    if not values:
        return
    with create_connection() as connection:
        connection.executemany(
            ...
            ON CONFLICT(key) DO UPDATE SET value = excluded.value
            """,
            [(key.strip(), str(value)) for key, value in values.items()],
        )
        connection.commit()
...
def get_app_settings() -> AppSettings:
    ...
    sync_enabled_raw = (get_setting("cloud_sync_enabled") or _DEFAULTS["cloud_sync_enabled"]).strip().lower()
    cloud_sync_enabled = sync_enabled_raw not in {"0", "false", "no"}
    cloud_sync_provider = (get_setting("cloud_sync_provider") or _DEFAULTS["cloud_sync_provider"]).strip()
    interval_raw = get_setting("cloud_sync_interval_minutes") or _DEFAULTS["cloud_sync_interval_minutes"]
    try:
        cloud_sync_interval_minutes = max(1, min(1440, int(interval_raw)))
    except ValueError:
        cloud_sync_interval_minutes = int(_DEFAULTS["cloud_sync_interval_minutes"])

    raw_sync_settings = get_setting("cloud_sync_settings_json") or _DEFAULTS["cloud_sync_settings_json"]
    try:
        cloud_sync_config_raw = json.loads(raw_sync_settings)
    except json.JSONDecodeError:
        cloud_sync_config_raw = {}
    if isinstance(cloud_sync_config_raw, dict):
        cloud_sync_config = {str(key): str(value) for key, value in cloud_sync_config_raw.items()}
    else:
        cloud_sync_config = {}
    ...
    return AppSettings(
        ...
        cloud_sync_interval_minutes=cloud_sync_interval_minutes,
