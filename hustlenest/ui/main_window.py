...
from ..data.database import close_database_for_replacement
...
from ..services import cloud_sync_service, order_service, soft_delete_service
...
from .business_tools_dialog import BusinessToolsDialog
from .cloud_sync_dialog import CloudSyncDialog
from .collapsible_group import CollapsibleGroupBox, get_section_manager
...
from .settings_dialogs import AdvancedSettingsDialog, ReportExportDialog
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._cloud_sync_timer: Optional[QTimer] = None
        self._cloud_sync_status_message: str = ""
        self._cloud_sync_status_is_error: bool = False
        self._perform_initial_cloud_sync()
        ...
        self._business_tools_dialog: Optional[BusinessToolsDialog] = None
        ...
        self._paid_sales_label: QLabel
        self._unpaid_sales_label: QLabel
        ...
        self._tax_year_input: QSpinBox
        self._tax_period_combo: QComboBox
        self._tax_summary_label: QLabel
        self._tax_export_status_label: QLabel
        self._editing_line_index: Optional[int] = None
        self._current_report_rows: List[OrderReportRow] = []

        self._cloud_sync_status_label: QLabel
        ...
        self._update_graphs()
        self._refresh_map()
        self._refresh_cloud_sync_timer()
    ...
    def _build_dashboard_tab(self) -> None:
        ...
        self._total_sales_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        header_layout.addWidget(self._total_sales_label)

        self._paid_sales_label = QLabel("Paid: $0.00")
        self._paid_sales_label.setStyleSheet("font-size: 14px; color: green;")
        header_layout.addWidget(self._paid_sales_label)

        self._unpaid_sales_label = QLabel("Unpaid: $0.00")
        self._unpaid_sales_label.setStyleSheet("font-size: 14px; color: orange;")
        header_layout.addWidget(self._unpaid_sales_label)

        self._net_sales_label = QLabel("Net Revenue: $0.00 (Freebies: $0.00)")
        self._net_sales_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        header_layout.addWidget(self._net_sales_label)

        self._outstanding_label = QLabel("Outstanding Orders: 0")
        self._outstanding_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        header_layout.addWidget(self._outstanding_label)

        self._completed_label = QLabel("Completed Orders: 0")
        self._completed_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        ...
        tools_button = QPushButton("Business Tools")
        tools_button.clicked.connect(self._open_business_tools)
        header_layout.addWidget(tools_button)
        ...
        metrics_layout.addWidget(self._wrap_group("Product Sales Breakdown", self._product_table, "product_sales"))
        metrics_layout.addWidget(self._wrap_group("Top Customers", self._customer_table, "top_customers"))
        metrics_layout.addWidget(self._wrap_group("Notifications", self._notifications_table, "notifications"))
        ...
        orders_layout.addWidget(self._wrap_group("Outstanding Orders", self._outstanding_table, "outstanding_orders"))
        orders_layout.addWidget(self._wrap_group("Completed Orders", self._completed_table, "completed_orders"))
    ...
    def _build_reports_tab(self) -> None:
        ...
        export_reports_button = QPushButton("Export Reports...")
        export_reports_button.clicked.connect(self._open_report_export_dialog)
        filter_layout.addWidget(export_reports_button)

        filter_layout.addStretch(1)

        tax_group = QGroupBox("Sales Tax Summary")
        tax_group_layout = QVBoxLayout()
        tax_group.setLayout(tax_group_layout)

        tax_controls = QHBoxLayout()
        tax_group_layout.addLayout(tax_controls)

        tax_controls.addWidget(QLabel("Year"))
        self._tax_year_input = QSpinBox()
        self._tax_year_input.setRange(2000, 2100)
        self._tax_year_input.setValue(QDate.currentDate().year())
        tax_controls.addWidget(self._tax_year_input)

        tax_controls.addWidget(QLabel("Period"))
        self._tax_period_combo = QComboBox()
        self._tax_period_combo.addItem("Full Year", "year")
        self._tax_period_combo.addItem("Q1 (Jan-Mar)", "q1")
        self._tax_period_combo.addItem("Q2 (Apr-Jun)", "q2")
        self._tax_period_combo.addItem("Q3 (Jul-Sep)", "q3")
        self._tax_period_combo.addItem("Q4 (Oct-Dec)", "q4")
        tax_controls.addWidget(self._tax_period_combo)

        tax_controls.addStretch(1)

        tax_csv_button = QPushButton("Export Tax CSV")
        tax_csv_button.clicked.connect(self._handle_export_tax_csv)
        tax_controls.addWidget(tax_csv_button)

        tax_pdf_button = QPushButton("Export Tax PDF")
        tax_pdf_button.clicked.connect(self._handle_export_tax_pdf)
        tax_controls.addWidget(tax_pdf_button)

        self._tax_summary_label = QLabel("Select a tax period to preview totals.")
        self._tax_summary_label.setStyleSheet("font-weight: bold;")
        tax_group_layout.addWidget(self._tax_summary_label)

        self._tax_export_status_label = QLabel()
        tax_group_layout.addWidget(self._tax_export_status_label)

        layout.addWidget(tax_group)

        self._tax_year_input.valueChanged.connect(self._on_tax_period_controls_changed)
        self._tax_period_combo.currentIndexChanged.connect(self._on_tax_period_controls_changed)
        ...
        self._update_tax_summary_preview()
    ...
    def _build_graphs_tab(self) -> None:
        ...
        header_layout.addWidget(QLabel("Chart Type:"))
        self._chart_type_combo = QComboBox()
        self._chart_type_combo.addItem("Product Sales (Bar Chart)", "bar_product_sales")
        self._chart_type_combo.addItem("Revenue Trend (Line Chart)", "line_revenue_trend")
        self._chart_type_combo.addItem("Sales by Status (Pie Chart)", "pie_status")
        self._chart_type_combo.addItem("Top Customers (Bar Chart)", "bar_customers")
        self._chart_type_combo.currentIndexChanged.connect(self._on_chart_type_changed)
        header_layout.addWidget(self._chart_type_combo)
        
        header_layout.addStretch(1)
        
        refresh_btn = QPushButton("Refresh Charts")
        refresh_btn.clicked.connect(lambda: self._on_chart_type_changed(self._chart_type_combo.currentIndex()))
    ...
    def _on_chart_type_changed(self, index: int) -> None:
        ...
        if chart_type == "bar_product_sales":
            self._update_graphs()
        elif chart_type == "line_revenue_trend":
            self._update_line_chart()
        elif chart_type == "pie_status":
            self._update_pie_chart()
        elif chart_type == "bar_customers":
            self._update_customer_bar_chart()
    ...
    def _build_settings_tab(self) -> None:
        ...
        sync_button = QPushButton("Open Cloud Sync Settings…")
        sync_button.clicked.connect(self._open_cloud_sync_dialog)
        layout.addWidget(sync_button, alignment=Qt.AlignmentFlag.AlignLeft)

        advanced_button = QPushButton("Advanced Settings (Theme, Backup, Import)…")
        ...
        deleted_button = QPushButton("Recently Deleted (Trash)…")
        deleted_button.clicked.connect(self._open_recently_deleted)
        layout.addWidget(deleted_button, alignment=Qt.AlignmentFlag.AlignLeft)

        self._cloud_sync_status_label = QLabel(self._cloud_sync_status_message)
        self._cloud_sync_status_label.setWordWrap(True)
        if self._cloud_sync_status_is_error:
            self._cloud_sync_status_label.setStyleSheet("color: #b00020;")
        else:
            self._cloud_sync_status_label.setStyleSheet("")
        layout.addWidget(self._cloud_sync_status_label)
        layout.addSpacing(6)
    ...
    def _build_about_tab(self) -> None:
        ...
        about_html = (
            ...
            "<ul>"
            "<li>Losses and expenses trackers now share the same category dropdown as materials for consistent tagging.</li>"
            "<li>Sales tax summaries generate quarterly or annual reports with direct CSV and PDF exports.</li>"
            "<li>CRM tab can import customer records from historical orders to jump-start outreach.</li>"
            "</ul>"
            f"<p>Project home: <a href=\"{REPOSITORY_URL}\">{REPOSITORY_URL}</a></p>"
        )
        about_browser.setHtml(about_html)
        layout.addWidget(about_browser, 1)
    ...
    def _check_for_updates(self) -> None:
        ...
        if result.is_newer and result.latest_version:
            ...
            remind_btn = msg_box.addButton("Remind Me Later", QMessageBox.ButtonRole.NoRole)
            ...
            if clicked == skip_btn:
                ...
                settings_repository.set_setting("skipped_update_version", result.latest_version)
    ...
    def refresh_dashboard(self) -> None:
        ...
        self._paid_sales_label.setText(f"Paid: ${snapshot.paid_sales:,.2f}")
        self._paid_sales_label.setToolTip("Total sales from orders marked as paid.")
        self._unpaid_sales_label.setText(f"Unpaid: ${snapshot.unpaid_sales:,.2f}")
        self._unpaid_sales_label.setToolTip("Total sales from orders not yet paid.")
    ...
    def _on_tax_period_controls_changed(self) -> None:
        if hasattr(self, "_tax_export_status_label"):
            self._tax_export_status_label.setText("")
            self._tax_export_status_label.setStyleSheet("")
        self._update_tax_summary_preview()

    def _resolve_tax_period(self) -> Optional[Tuple[date, date, str]]:
        if not hasattr(self, "_tax_year_input") or not hasattr(self, "_tax_period_combo"):
            return None

        year = self._tax_year_input.value()
        mode = self._tax_period_combo.currentData()
        if mode == "year":
            start = date(year, 1, 1)
            end = date(year, 12, 31)
            label = f"{year}"
    ...
    def _update_tax_summary_preview(self) -> None:
        if not hasattr(self, "_tax_summary_label"):
            return

        period = self._resolve_tax_period()
        if period is None:
            self._tax_summary_label.setText("Select a valid tax period.")
            self._tax_summary_label.setStyleSheet("font-weight: bold; color: #d32f2f;")
            return

        start, end, label = period
        try:
            summary, _ = order_service.summarize_sales_tax(start, end, period_label=label)
        except Exception as exc:  # noqa: BLE001
            self._tax_summary_label.setText(f"Tax summary unavailable: {exc}")
            self._tax_summary_label.setStyleSheet("font-weight: bold; color: #d32f2f;")
            return

        taxable_text = f"${summary.taxable_sales:,.2f}"
        collected_text = f"${summary.tax_collected:,.2f}"
        expected_text = f"${summary.expected_tax:,.2f}"
        ...
        summary_text = (
            f"{label}: {summary.order_count} invoices | "
            f"Taxable {taxable_text} | Collected {collected_text} | "
            f"Expected {expected_text} | Difference {diff_text}"
        )
        self._tax_summary_label.setText(summary_text)
        self._tax_summary_label.setStyleSheet(f"font-weight: bold; color: {color};")

    def _handle_export_tax_csv(self) -> None:
        self._export_tax_summary(kind="csv")

    def _handle_export_tax_pdf(self) -> None:
        self._export_tax_summary(kind="pdf")

    def _export_tax_summary(self, *, kind: str) -> None:
        period = self._resolve_tax_period()
        if period is None:
            self._tax_export_status_label.setStyleSheet("color: #d32f2f;")
            self._tax_export_status_label.setText("Set a valid tax year and period before exporting.")
            return

        start, end, label = period
        business_slug = (self._app_settings.business_name or "HustleNest").strip() or "HustleNest"
        business_slug = business_slug.replace(" ", "_")
        label_slug = label.replace(" ", "_")
        if kind == "csv":
            ext = "csv"
            title = "Export Sales Tax CSV"
            filter_text = "CSV Files (*.csv)"
            export_func = order_service.export_sales_tax_summary_csv
        else:
            ext = "pdf"
            title = "Export Sales Tax PDF"
            filter_text = "PDF Files (*.pdf)"
            export_func = order_service.export_sales_tax_summary_pdf

        default_name = f"{business_slug}_SalesTax_{label_slug}.{ext}"
        suggested_path = str(Path.home() / default_name)
        filename, _ = QFileDialog.getSaveFileName(self, title, suggested_path, filter_text)
        if not filename:
            return

        try:
            exported_path = export_func(start, end, filename, period_label=label)
        except Exception as exc:  # noqa: BLE001
            self._tax_export_status_label.setStyleSheet("color: #d32f2f;")
            self._tax_export_status_label.setText(f"Export failed: {exc}")
            return

        self._tax_export_status_label.setStyleSheet("color: #2e7d32;")
        self._tax_export_status_label.setText(f"Sales tax summary saved to {exported_path}")
    ...
    def _open_business_tools(self) -> None:
        if self._business_tools_dialog is None:
            dialog = BusinessToolsDialog(self)
            dialog.destroyed.connect(self._on_business_tools_destroyed)
            self._business_tools_dialog = dialog

        dialog = self._business_tools_dialog
        if dialog is None:
            return
    ...
    def _on_business_tools_destroyed(self, _obj: Optional[object] = None) -> None:
        self._business_tools_dialog = None
    ...
    def _handle_delete_order(self) -> None:
        ...
        confirm = QMessageBox.question(
            self,
            APP_NAME,
            f"Move order {self._selected_order.order_number} to trash?\n\nYou can restore it from Settings > Recently Deleted.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        ...
        try:
            ...
            if not soft_delete_service.soft_delete_order(order_id):
                self._set_order_status("Selected order could not be found.", error=True)
                self._load_recent_orders()
                return
        except Exception as exc:  # noqa: BLE001
        ...
        self._set_order_status("Order moved to trash.")
        self.refresh_dashboard()
        self._refresh_products()
        self._load_recent_orders()
        self._run_report()
    ...
    def _handle_cloud_sync_tick(self) -> None:
        outcome = cloud_sync_service.upload_database(self._app_settings)
        self._apply_cloud_sync_outcome(outcome)

    def _apply_cloud_sync_outcome(
        self,
        outcome: cloud_sync_service.SyncOutcome,
        *,
        refresh_settings: bool = False,
        update_timer: bool = True,
    ) -> None:
        message = outcome.message or ("Sync completed." if outcome.success else "Cloud sync failed.")
        self._cloud_sync_status_message = message

        if outcome.success:
            self._cloud_sync_status_is_error = False
        else:
            self._cloud_sync_status_is_error = True

        if hasattr(self, "_cloud_sync_status_label"):
            if self._cloud_sync_status_is_error:
                self._cloud_sync_status_label.setStyleSheet("color: #b00020;")
            else:
                self._cloud_sync_status_label.setStyleSheet("")
            self._cloud_sync_status_label.setText(message)

        if refresh_settings or outcome.success:
            self._app_settings = order_service.get_app_settings()

        if outcome.downloaded and outcome.success:
            self.refresh_dashboard()
            self._load_recent_orders()
    ...
    def _refresh_cloud_sync_timer(self) -> None:
        if self._cloud_sync_timer is not None:
            self._cloud_sync_timer.stop()

        if not cloud_sync_service.is_configured(self._app_settings):
            return

        interval_seconds = cloud_sync_service.sync_interval_seconds(self._app_settings)
        if self._cloud_sync_timer is None:
            self._cloud_sync_timer = QTimer(self)
            self._cloud_sync_timer.timeout.connect(self._handle_cloud_sync_tick)
        self._cloud_sync_timer.start(interval_seconds * 1000)

    def _stop_cloud_sync_timer(self) -> None:
        if self._cloud_sync_timer is not None:
            self._cloud_sync_timer.stop()
    ...
    def _perform_initial_cloud_sync(self) -> None:
        if not cloud_sync_service.is_configured(self._app_settings):
            self._cloud_sync_status_message = "Cloud sync disabled."
            self._cloud_sync_status_is_error = False
            return

        # Close any database connections before attempting to replace the database file
        close_database_for_replacement()

        outcome = cloud_sync_service.download_database_if_newer(self._app_settings)
        if outcome.success:
            self._cloud_sync_status_message = outcome.message or "Cloud sync ready."
            self._cloud_sync_status_is_error = False
            if outcome.downloaded:
                self._app_settings = order_service.get_app_settings()
        else:
            self._cloud_sync_status_message = outcome.message or "Cloud sync failed."
            self._cloud_sync_status_is_error = True
    ...
    def _handle_save_settings(self) -> None:
        ...
        if hasattr(self, "_cloud_sync_status_label"):
            self._cloud_sync_status_label.setStyleSheet("")
        self._cloud_sync_status_message = "Settings saved."
        if hasattr(self, "_cloud_sync_status_label"):
            self._cloud_sync_status_label.setText(self._cloud_sync_status_message)
        self.refresh_dashboard()
        self._run_report()
        self._refresh_products()
        self._refresh_cloud_sync_timer()
    ...
    def _open_cloud_sync_dialog(self) -> None:
        dialog = CloudSyncDialog(self._app_settings, parent=self)
        result = dialog.exec()
        updated_settings = dialog.get_updated_settings()
        if result == QDialog.DialogCode.Accepted and updated_settings is not None:
            self._app_settings = updated_settings
            self._settings_status_label.setText("Cloud sync settings saved.")
            self._cloud_sync_status_is_error = False
            if hasattr(self, "_cloud_sync_status_label"):
                self._cloud_sync_status_label.setStyleSheet("")
                self._cloud_sync_status_label.setText("Cloud sync settings saved.")
            self._cloud_sync_status_message = "Cloud sync settings saved."
            self._refresh_cloud_sync_timer()

        last_outcome = dialog.get_last_outcome()
        if last_outcome is not None:
            self._apply_cloud_sync_outcome(last_outcome, refresh_settings=True)

    def _open_advanced_settings(self) -> None:
        """Open the Advanced Settings dialog with theme, backup, and import options."""
        dialog = AdvancedSettingsDialog(self._app_settings, parent=self)
        dialog.exec()
        # Refresh products in case an import was performed
        self._refresh_products()

    def _open_report_export_dialog(self) -> None:
        """Open the Report Export dialog for generating PDF reports."""
        dialog = ReportExportDialog(self._app_settings, parent=self)
        dialog.exec()

    def _open_recently_deleted(self) -> None:
        """Open the Recently Deleted dialog for viewing/restoring deleted items."""
        dialog = RecentlyDeletedDialog(parent=self)
        dialog.exec()
        # Refresh data in case items were restored
        self._refresh_products()
    ...
    def _update_line_chart(self) -> None:
        """Create a line chart showing revenue trend over the last 30 days."""
        from ..data import order_repository
        
        chart = QChart()
        chart.setTitle("Revenue Trend (Last 30 Days)")
        ...
        # Aggregate by date
        orders = order_repository.fetch_orders(limit=500)
        daily_totals: Dict[date, float] = {}
        
        for order in orders:
            order_date = order.order_date
            if start_date <= order_date <= end_date:
                current = daily_totals.get(order_date, 0.0)
                daily_totals[order_date] = current + order.total_amount
        ...
        # Sort and add to series
        sorted_dates = sorted(daily_totals.keys())
        categories: List[str] = []
        
        for d in sorted_dates:
            # Use day index for X axis
            day_index = (d - start_date).days
            series.append(day_index, daily_totals[d])
            if d.day == 1 or d == sorted_dates[0] or d == sorted_dates[-1]:
        ...
        axis_y = QValueAxis()
        axis_y.setLabelFormat("$%.0f")
        max_val = max(daily_totals.values()) if daily_totals else 100
    ...
    def _update_pie_chart(self) -> None:
        """Create a pie chart showing sales by order status."""
        from ..data import order_repository
        
        chart = QChart()
        chart.setTitle("Sales by Order Status")
        
        series = QPieSeries()
        
        # Get orders and aggregate by status
        orders = order_repository.fetch_orders(limit=500)
        status_totals: Dict[str, float] = {}
        
        for order in orders:
            status = order.status or "Unknown"
            current = status_totals.get(status, 0.0)
            status_totals[status] = current + order.total_amount
        
        if not status_totals:
            status_totals = {"No Data": 1.0}
        
        # Add slices
        colors = ["#4CAF50", "#2196F3", "#FFC107", "#F44336", "#9C27B0", "#00BCD4"]
        for idx, (status, amount) in enumerate(sorted(status_totals.items(), key=lambda x: -x[1])):
            slice_item = series.append(f"{status}: ${amount:,.0f}", amount)
            if slice_item:
    ...
    def _update_customer_bar_chart(self) -> None:
        """Create a bar chart showing top customers by sales."""
        snapshot = order_service.get_dashboard_snapshot()
        data = list(snapshot.top_customers) if snapshot.top_customers else []
        
        chart = QChart()
        chart.setTitle("Top Customers by Sales")
        ...
        for row in data[:10]:
            name = getattr(row, "customer_name", "Unknown")
            # Truncate long names
            if len(name) > 15:
                name = name[:12] + "..."
    ...
    def _wrap_group(self, title: str, widget: QWidget, section_key: str = "") -> QWidget:
        """Wrap a widget in a collapsible group box.
        
        Args:
            title: The title for the group box header
            widget: The content widget to wrap
            section_key: Optional key for persisting collapse state. If empty,
                        uses a non-collapsible simple container.
        """
        if section_key:
            # Use collapsible group box with persistence
            group = CollapsibleGroupBox(title, section_key, self)
            group.set_content(widget)
            # Register with section manager
            get_section_manager().register_section(section_key, group)
            return group
        else:
            # Simple non-collapsible container (legacy behavior)
            container = QWidget()
            layout = QVBoxLayout()
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        if cloud_sync_service.is_configured(self._app_settings):
            outcome = cloud_sync_service.upload_database(self._app_settings)
            self._apply_cloud_sync_outcome(
                outcome,
                refresh_settings=outcome.success,
                update_timer=False,
            )
        self._stop_cloud_sync_timer()
